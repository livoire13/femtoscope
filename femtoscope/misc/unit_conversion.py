...
from femtoscope.misc.constants import C_LIGHT, H_BAR, EV, M_PL
...
def compute_alpha(Lambda, beta, npot, L_0, rho_0):
    ...
    t2 = npot*M_PL*Lambda**3 / (beta*rho_0) * (EV/(H_BAR*C_LIGHT))**3
...
def compute_phi_0(Lambda, beta, npot, rho_0):
    ...
    Lambda : [eV]
    """
    t = npot*M_PL*Lambda**(npot+4)/(beta*rho_0) * (EV/(H_BAR*C_LIGHT))**3
    return t**(1/(npot+1))
